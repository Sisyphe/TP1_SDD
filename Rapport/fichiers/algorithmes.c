...
fonction inserEvent(e/s : adr, e : year, week)
    m(newWeek) = newWeekCal(year, week)

    m(newWeek) = adr;
    m(newWeek)+1 = cm(adr)+1;
    m(cm(adr)+1) = cm(newWeek);
    m(cm(newWeek)+1) = newWeek;

    retourner cm(newWeek);
...
fonction inserEvent(e/s : adrPrev, e : day, hour, event)
    m(newEvent) = newEventCal(day, hour, event)

    m(newEvent) = adrPrev;
    m(cm(adrPrev)) = cm(newEvent);

    retourner cm(newEvent);
...
fonction newWeekCal(e : day, hour, event)
    m(newWeek) = allouer(9);

    m(newWeek) = cm(newWeek); // prevWeek
    m(newWeek)+1 = cm(newWeek); // nextWeek
    m(newWeek)+2 = nil; // eventList
    m(newWeek)+3 = year;
    m(newWeek)+7 = week;
...
fonction newEventCal(e : day, hour, event)
    m(newEvent) = allouer(14);

    m(newEvent) = nil; // nextEvent
    m(newEvent)+1 = day;
    m(newEvent)+2 = hour;
    m(newEvent)+4 = event;
